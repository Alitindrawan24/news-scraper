...
function getTime($) {
    let time = $(".read-page--header--author__datetime");
...
function getTitle($) {
    return $(".read-page--header--title").text().replaceAll("\n", "").replaceAll(' ', " ").trim()
}

function getContent($) {
    let content = $(".article-content-body__item-content");
    $("script", content).remove();

    content = content.text()

    let readToo = $(".baca-juga").text();
    ...
    content = content.replaceAll("Liputan6.com, ", "").trim()
