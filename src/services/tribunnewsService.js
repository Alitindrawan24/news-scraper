...
async function getData(url) {
    let response = await axios.get(url, { rejectUnauthorized: false })
    ...
    // check page
    let page = 2
    let pageCount = $('.paging .ovh a').text()
...
function getTitle($) {
    return $("#arttitle").text().replaceAll("\n", "").replaceAll(' ', " ").trim()
}

function getContent($) {
    let content = $("#article_con .txt-article");
    $("script", content).remove();
    $("figure", content).remove();
